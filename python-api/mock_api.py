...
def looks_medical_filename(filename: str) -> bool:
    name = filename.lower()
    medical_terms = ["xray", "x-ray", "radiograph", "mri", "ct", "scan", "dicom"]
...
async def predict_upload(patientId: str = Form(...), image: UploadFile = File(...)):
    ...
    # Optional lightweight filename-based check
    if not looks_medical_filename(filename) and suffix not in {".dcm", ".dicom"}:
        raise HTTPException(
            status_code=400,
            detail="This file does not appear to be a supported medical image.",
        )
    ...
    try:
        ...
        # Real inference logic would go here, using the saved file at save_path
